...
export default function BuyNowPage() {
  ...
  useEffect(() => {
    ...
    GetSubscription(session?.user?.id, session?.accessToken).then((res) => {
      ...
      if (res?.[0]) {
        console.log(res?.[0])
        setPlanFrequency(res?.[0]?.plan?.planFrequency)
        setPlanName(res?.[0]?.plan?.name)
        setSelectedCycle(res?.[0]?.planFrequency)

        setStartingPrice(parseFloat(res?.[0]?.plan?.startingPrice))

        setAutoRenew(res?.[0]?.autoRenewal)

        setTotalUser([
          ...res?.[0]?.teams?.map((team) => {
            return {
              id: team?.id,
              email: team?.username,
            }
        ...
        setPlanStartDate(moment(res?.[0]?.validFrom).format("YYYY-MM-DD"))
        setPlanEndDate(moment(res?.[0]?.validTo).format("YYYY-MM-DD"))
 ...
 const goToBack = () => {
   localStorage.setItem("selectedTab", "MY_SUBSCRIPTION")
     router.push("/profile")
 }
  return (
    <>
          ...
          <div className="w-full lg:w-[60%] pb-[136px] relative">
            <div className="mt-36 pr-5 sm:pr-10 lg:pr-5 pl-5 sm:pl-12  md:pl-20">
              <div className="text-sm text-[#A6A6A6] cursor-pointer mb-2 sm:mb-0">
                ...
                <div className="flex items-center cursor-pointer w-fit" onClick={goToBack}>
                  <FaAngleLeft /> <span className="ml-1">Back</span>
                </div>
              </div>
              <div className="mt-10">
